...
sep_data_indiffY_sepMismatch<-function(ddf,degf,Clim,PFT,site_name,avai_years){
  ...
  Years_unique<-unique(df$Year)
    ...
    #additional filter: only selecting the time period in the first 2/3 green-up for the determination-->
    ...
    # }
    #method:classify the position of pos_isevent when air temperature >10
...
```{r echo = FALSE, message = FALSE, warning = FALSE,fig.height=10,fig.width=10}
#US-Syv
df_andPlot_Dfb_MF<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","MF","US-Syv",c(2002:2006,2014))
...
save(ddf_labeled,file=paste0(save.path,"ddf_labeled_norm_trs_newmethod.RDA"))
save(df_all,file=paste0(save.path,"df_all_results/df_all_norm_trs_newmethod.RDA"))
...
### steps to determine the "is_event" period
    ...
    **Step 3:rolling mean of GPP_obs and GPP_mod for data for all the years (moving windown:5,7,10, 15, 20days)**
    ...
    **step 6:Evaluation "is_event"-->visualization and stats**
     ...
     - two ways to evaluate if "is_event" is properly determined:
      ...
      1) visulization
      green lines:sos and peak of the GPP_mod time series
      blue lines:GPP overstimation (when GPP_mod > GPP_obs)
      orange rectangle: GPP overestimation and when the temperature smaller than 10 degrees Celsius
