...
**Try to prepare some plot to demonstrate what I was doing**

## seprate the time period when model early estimation of GPP
...
sep_data_indiffY_sepMismatch<-function(ddf,degf,Clim,PFT,site_name,avai_years){
  ...
  Years_unique<-unique(df$Year)
    ...
    pos_isevent_less10<-df_subset$doy[which(df_subset$is_event=="yes")]
    pos_isevent_less10<-df_subset$doy[which(df_subset$is_event_less10=="yes")]
    
    rect.coord_GS=data.frame(x1=df_subset$date[pos_agg[,"sos10"]],x2=df_subset$date[pos_agg[,"peak"]], y1=min(df_subset$gpp_mod_norm,na.rm = T)-0.05, y2=max(df_subset$gpp_mod_norm,na.rm = T)+0.02)
...
#
##plot1: only to show the sos, peak, and eos 
  gg_plot_pheons<-ggplot() + 
      ...
      geom_vline(xintercept = df_subset$date[pos_agg[,"eos10"]],col="forestgreen",lty=2,lwd=1.2)+
  ...
  ##plot2: adding different ranges  
  gg_plot_diffranges<-ggplot() + 
      geom_point(data=df_subset,aes(x=date,y = gpp_mod_norm),col=adjustcolor("tomato",0.5),pch=1)+
      geom_line(data=df_subset,aes(x=date,y = gpp_mod_roll20,col="modelled"),lwd=1.2)+
      geom_point(data=df_subset,aes(x=date,y = gpp_obs_norm),col=adjustcolor("black",0.5),pch=1)+
      geom_line(data=df_subset,aes(x=date,y = gpp_obs_roll20,col="observed"),lwd=1.2)+
...
#save the data:
# save(ddf_labeled,file=paste0(path,"ddf_labeled_norm_trs_newmethod_all_overestimation.RDA"))
# save(df_all,file=paste0(path,"df_all_results/df_all_norm_trs_newmethod_all_overestimation.RDA"))
