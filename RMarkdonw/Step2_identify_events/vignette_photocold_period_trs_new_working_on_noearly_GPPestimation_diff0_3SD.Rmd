...
#path to merge datatasets:
path<-"C:/Users/yluo/Desktop/CES/Data_for_use/Merge_Data/"
...
sep_data_indiffY_sepMismatch<-function(ddf,degf,Clim,PFT,site_name,avai_years){
  ...
  # PFT<-"ENF"
  # site_name<-"IT-Ren"
  # avai_years<-c(2002:2003,2005:2013)
  #
  ##
  df<-ddf %>%
  ...
  Years_unique<-unique(df$Year)
    ...
    #remove some extremes-->avoid large effects on distribution
#remove some extremes in NL-Hor-->avoid large effects on distribution
    ...
    if(length(df_subset[c(df_subset$GS=="yes")&c(df_subset$gpp_res>thrs)&c(!is.na(df_subset$gpp_res)),]$over_estim)>0){
    ...
    #for the data is not equal to NA
    if(length(df_subset[c(df_subset$greenup=="yes")&c(df_subset$gpp_res>thrs)&c(!is.na(df_subset$gpp_res)),]$is_event)>0){
    ...
    #method:remove the positon of "is_event" when air temperature >10 (should be in the green-up period)
...
```{r echo = FALSE, message = FALSE, warning = FALSE,fig.height=10,fig.width=10}
#BE-Vie
df_andPlot_Cfb_MF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","MF","BE-Vie",c(2000:2014))
...
#CH-Lae
df_andPlot_Cfb_MF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","MF","CH-Lae",c(2005:2014))
...
names(df_all)<-c("CH-Cha","CH-Oe1","DE-Gri","DE-RUR","NL-Hor","DE-Obe","DE-Tha","NL-Loo","BE-Vie","CH-Lae","IT-Ren")
ddf_labeled<-c()
for(i in 1:length(df_all)){
  ddf_labeled<-rbind(ddf_labeled,df_all[[i]]$df_agg)
}
#save the data:
save.path<-"C:/Users/yluo/Desktop/CES/Data_for_use/Data_sent_by_Beni/"
save(ddf_labeled,file=paste0(save.path,"ddf_labeled_norm_trs_newmethod_sites_flag_without_overestimation.RDA"))
save(df_all,file=paste0(save.path,"df_all_results/df_all_norm_trs_newmethod_flag_without_overestimation.RDA"))
