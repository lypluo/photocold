---
title: "Align plot for different PFTs"
author: "Yunpeng Luo"
date: "16/12/2021"
output: html_document
---
Description: comparing the variables in "event" and "non-event" site years after aligning the data
using the data from oringal P-model gpp
...
for(i in 1:length(df_len5_nonnorm)){
  ...
  df_proc$LUE<-df_proc$gpp_obs/c(df_proc$fapar_itpl*df_proc$ppfd_fluxnet2015) #gpp_obs-->gpp
...
plot_2groups_PFT<-function(df,comp_var,var_unit,do_norm,do_legend,PFT_name){
    ...
    p_plot<-ggplot(df.all_q)+
      ...
      theme(legend.position = c(0.35,0.9),legend.background = element_blank(),
            legend.text = element_text(size=10),  #size 20-->10
            axis.title = element_text(size=12),   #size 24-->12
            axis.text = element_text(size = 10))+ #size 20-->10
...
merge_plot_fun<-function(out_all,save.path,PFT_name){
  ...
  p_merge_Cflux<-plot_grid(out_all$p_gpp_obs_len5_b60$plot,
                           ...
                           labels = "auto",nrow=2,label_size = 16,align = "hv")
  print(p_merge_Cflux)
...
df.event.DBF<-df.event %>% filter(classid==PFT_name)
df.noevent.DBF<-df.noevent %>% filter(classid==PFT_name)
print(paste0("DBF event observations are:",nrow(df.event.DBF)))
print(paste0("DBF noevent observations are:",nrow(df.noevent.DBF)))
...
df.event.MF<-df.event %>% filter(classid==PFT_name)
df.noevent.MF<-df.noevent %>% filter(classid==PFT_name)
print(paste0("MF event observations are:",nrow(df.event.MF)))
print(paste0("MF noevent observations are:",nrow(df.noevent.MF)))
merge_plot_fun(out_MF,save.path,"DBF")
```
...
df.event.ENF<-df.event %>% filter(classid==PFT_name)
df.noevent.ENF<-df.noevent %>% filter(classid==PFT_name)
print(paste0("ENF event observations are:",nrow(df.event.ENF)))
print(paste0("ENF noevent observations are:",nrow(df.noevent.ENF)))
