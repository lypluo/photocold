...
##########################################
#------------------------------------
#(1)load the daily flux-sites data (fluxes data are from differernt source:
# Beni processed and Yunpeng processed )
#------------------------------------
library(lubridate)
library(dplyr)
...
#check the data availability in Beni's datasets
apply(df_avai, 2,function(x){length(x[!is.na(x)])/length(x)})


#c. keep the variables that Beni's datasets(df_avai) have, and add other variables from df_YP_daily
...
save.path<-"D:/data/photocold_project/Merge_Data/Using_sites_in_Fluxnet2015_constant_LUEmodel/"
