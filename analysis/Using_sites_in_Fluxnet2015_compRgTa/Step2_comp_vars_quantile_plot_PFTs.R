...
#do_vars-->the variables that are going to be normalized-->the normalized variables added at the end of the datasets
#in this study-->set do_norm=FALSE-->hence do not conduct this analysis:
names(df_norm_all)
...
#!write the function that works for different PFTs
plot_2groups_PFT<-function(df,comp_var,var_unit,do_norm,do_legend,PFT_name){
    # df<-df_len5_nonnorm
    ...
    #specify the PFT type:
    ##!!pay attention: the df.noevent-->for DBF only 334 points-->2 sites-->hence might have some biaes conclusion
    ...
    #!!add comments for stat-->2021-12-13:
    #-->since sites number in some variable is 2-->it is hard to do statstial comaparison, e.g. for tmin
    #-->hence comment for the stat comparsion here: 
    stat.q50_for2<-NULL
    stat.alldata_for2<-NULL
...
analysis_PFTs<-function(df_len5_nonnorm,PFT_name){
  ...
  #-------------
  # #gpp_obs
  # p_gpp_obs_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"gpp_obs","(umol m-2 s-1)",do_norm = FALSE,do_legend = TRUE,PFT_name)
  # #gpp biaes
  # p_gpp_biaes_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"gpp_res","(umol m-2 s-1)",do_norm = FALSE,FALSE,PFT_name)
  # #LUE
  # p_LUE_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"LUE","",do_norm = FALSE,FALSE,PFT_name)
  # #for ppfd
  # p_ppfd_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"ppfd_fluxnet2015","(u mol m-2 s-1)",do_norm = FALSE,do_legend = FALSE,PFT_name)
  # #fapar_spl and fapar_itpl
  # p_fapar_itpl_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"fapar_itpl","",do_norm = FALSE,do_legend = FALSE,PFT_name)
  ...
  #-------------
  #temp_day
  p_temp_day_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"temp_day_fluxnet2015","(degreeC)",do_norm = FALSE,FALSE,PFT_name)
  #temp_min
  p_temp_min_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"temp_min_fluxnet2015","(degreeC)",do_norm = FALSE,do_legend = TRUE,PFT_name)
  #temp_max
  ...
  #SW_IN_fullday_mean
  p_SW_IN_fullday_mean_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"SW_IN_fullday_mean_fluxnet2015","(W m-2)",do_norm = FALSE,FALSE,PFT_name)
  #SW_IN_midday_mean
  p_SW_IN_midday_mean_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"SW_IN_midday_mean_fluxnet2015","(W m-2)",do_norm = FALSE,FALSE,PFT_name)
  #SW_IN_midday_max
  p_SW_IN_midday_max_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"SW_IN_midday_max_fluxnet2015","(W m-2)",do_norm = FALSE,FALSE,PFT_name)
  #PPFD_fullday_mean
  p_PPFD_IN_fullday_mean_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"PPFD_IN_fullday_mean_fluxnet2015","(umol m-2 s-1)",do_norm = FALSE,FALSE,PFT_name)
  #PPFD_IN_midday_mean
  p_PPFD_IN_midday_mean_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"PPFD_IN_midday_mean_fluxnet2015","(umol m-2 s-1)",do_norm = FALSE,FALSE,PFT_name)
  #PPFD_IN_midday_max
  p_PPFD_IN_midday_max_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"PPFD_IN_midday_max_fluxnet2015","(umol m-2 s-1)",do_norm = FALSE,FALSE,PFT_name)
  ...
  #albedo_SW_IN
  p_alpha_SW_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"alpha_SW","",do_norm = FALSE,FALSE,PFT_name)
  #albedo_ppfd
  p_alpha_PPFD_len5_b60<-plot_2groups_PFT(df_len5_nonnorm,"alpha_PPFD","",do_norm = FALSE,FALSE,PFT_name)
  ...
  p_alpha_SW_len5_b60$plot<-p_alpha_SW_len5_b60$plot+
    ylab("alpha_SW")
  p_alpha_PPFD_len5_b60$plot<-p_alpha_PPFD_len5_b60$plot+
    ylab("alpha_PPFD")
  #--------------
  #III.PhenoCam VIs
  #-------------
  ...
  out_all<-list(p_temp_min_len5_b60=p_temp_min_len5_b60,
                p_temp_day_len5_b60=p_temp_day_len5_b60,
                # p_temp_max_len5_b60=p_temp_max_len5_b60,
                p_SW_IN_fullday_mean_len5_b60=p_SW_IN_fullday_mean_len5_b60,
                p_SW_IN_midday_mean_len5_b60=p_SW_IN_midday_mean_len5_b60,
                p_SW_IN_midday_max_len5_b60=p_SW_IN_midday_max_len5_b60,
                p_PPFD_IN_fullday_mean_len5_b60=p_PPFD_IN_fullday_mean_len5_b60,
                p_PPFD_IN_midday_mean_len5_b60=p_PPFD_IN_midday_mean_len5_b60,
                p_PPFD_IN_midday_max_len5_b60=p_PPFD_IN_midday_max_len5_b60,
                p_alpha_SW_len5_b60=p_alpha_SW_len5_b60,
                p_alpha_PPFD_len5_b60=p_alpha_PPFD_len5_b60 #Environmental factors
  )
...
#(6) save the plot
#-------------------------------------------------------------------------
save.path<-"D:/plots/photocold_project/Using_sites_in_Fluxnet2015_compRgTa/quantile_plot/For_different_PFTs/"
#for different sites:
out_DBF<-analysis_PFTs(df_len5_nonnorm,"DBF")
out_MF<-analysis_PFTs(df_len5_nonnorm,"MF")
out_ENF<-analysis_PFTs(df_len5_nonnorm,"ENF")

#merge plots
merge_plot_fun<-function(out_all,save.path,PFT_name){
  # out_all<-out_DBF
  # save.path<-save.path
  # PFT_name<-"DBF"
  #--------------
  ...
  # print(p_merge_Cflux)
  # ggsave(paste0(save.path,PFT_name,"_","p_Cflux.png"),p_merge_Cflux,width = 20,height = 15)
  #--------------
  #II.Environment variables
  #-------------
  p_merge_EnviroVars<-plot_grid(
    out_all$p_temp_day_len5_b60$plot,out_all$p_temp_min_len5_b60$plot,
    out_all$p_SW_IN_fullday_mean_len5_b60$plot,out_all$p_SW_IN_midday_mean_len5_b60$plot,out_all$p_SW_IN_midday_max_len5_b60$plot,
    out_all$p_PPFD_IN_fullday_mean_len5_b60$plot,out_all$p_PPFD_IN_midday_mean_len5_b60$plot,out_all$p_PPFD_IN_midday_max_len5_b60$plot,
    out_all$p_alpha_SW_len5_b60$plot,out_all$p_alpha_PPFD_len5_b60$plot,
    labels = "auto",ncol=2,label_size = 18,align = "hv")

    ggsave(paste0(save.path,PFT_name,"_","p_EnviroVars.png"),p_merge_EnviroVars,width = 15,height = 30)
  #--------------
  #III.PhenoCam VIs
  #-------------
  ...
  #                        labels = "auto",nrow=2,label_size = 18,align = "hv")
  # # print(p_merge_VIs)
  # ggsave(paste0(save.path,PFT_name,"_","p_VIs.png"),p_merge_VIs,width = 15,height = 10)
}

#
merge_plot_fun(out_DBF,save.path,"DBF")
merge_plot_fun(out_MF,save.path,"MF")
merge_plot_fun(out_ENF,save.path,"ENF")
