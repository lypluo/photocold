...
#----------------------------------------------------
#add by YP-2021-12-20:only select the ppfd,SW_IN, Ta to explore..
library(lubridate)
library(tidyverse)
...
subset_interest_vars<-function(proc.path,sel_site){
  ...
  #sel vars position:
  pos_all<-c(pos_TIMESTAMP,pos_TA,pos_SW_IN,pos_SW_OUT,pos_PPFD,
             pos_NEE_VUT_REF,pos_GPP_VUT_REF,pos_TS_F_MDS)
  #
  # if(!is.na(c(match("SW_OUT",vars_names)))){
  #   pos_SW_OUT<-c(match("SW_OUT",vars_names))
  #   pos_all<-c(pos_all,pos_SW_IN)
  # }
...
save.path<-"D:/data/photocold_project/FLUXNET_data/Using_sites_in_Fluxnet2015_compRgTa/"
...
sel_variables<-c("sitename","TIMESTAMP_START","TA_F","SW_IN_F","SW_OUT",
                 ...
                 paste0("TS_F_MDS_",c(1:7))) #from the datasets-->most layer of TS=7
