...
apply(df_merge_over,2,function(x){length(x[!is.na(x)])/length(x)})
#only keep the 8day values:
# df_merge_over_8day<-df_merge_over[!is.na(df_merge_over$gpp_bess_8day),]
#first have a look at the performance for each site or all sites
#b1. for each site:
df_merge_over %>%
  group_by(sitename,doy)%>%
  summarise(obs = mean(gpp_obs_8day, na.rm = TRUE),
            obs_sd = sd(gpp_obs_8day,na.rm = T),
            mod_conLUE = mean(gpp_conLUE_8day,na.rm=T),
            mod_conLUE_sd = sd(gpp_conLUE_8day,na.rm = T),
            mod_Pmodel = mean(gpp_mod_FULL_8day,na.rm=T),
            mod_Pmodel_sd = sd(gpp_mod_FULL_8day,na.rm = T),
            mod_BESS = mean(gpp_bess_8day,na.rm=T),
            mod_BESS_sd = sd(gpp_bess_8day,na.rm = T),
            mod_VPM = mean(gpp_vpm_8day,na.rm=T),
            mod_VPM_sd = sd(gpp_vpm_8day,na.rm = T),
            mod_RF = mean(gpp_rf_8day,na.rm=T),  
            mod_RF_sd = sd(gpp_rf_8day,na.rm = T)
            ) %>%
  pivot_longer(c(obs,mod_conLUE,mod_Pmodel,mod_BESS,mod_VPM,mod_RF),
...
df_gpp_over_sel<-df_gpp_over[,c("doy","obs_mean","mod_conLUE_mean","mod_Pmodel_mean",
                                "mod_BESS_mean","mod_VPM_mean","mod_RF_mean")]
t<-apply(df_gpp_over_sel[,-1],2,function(x){na.fill(x,"extend")})
df_gpp_over_final<-cbind(df_gpp_over_sel[,1],as.data.frame(t))
#
df_gpp_fit<-c()
for (i in 2:length(df_gpp_over_final)) {
  temp<-df_gpp_over_final[,i]
  temp_sm<-SplineFit(temp)
  temp_fit<-as.numeric(temp_sm$fit$predicted)
  df_gpp_fit<-cbind(df_gpp_fit,temp_fit)
}
df_gpp_fit<-as.data.frame(df_gpp_fit)
names(df_gpp_fit)<-names(df_gpp_over_sel[,-1])
df_gpp_fit_final<-cbind(df_gpp_over[,1],df_gpp_fit)
#
df_gpp_end1<-df_gpp_over_final %>%
  select(doy,obs_mean,mod_conLUE_mean,mod_Pmodel_mean,
         mod_BESS_mean,mod_VPM_mean,mod_RF_mean)%>%
  pivot_longer(c(obs_mean,mod_conLUE_mean,mod_Pmodel_mean,
                 mod_BESS_mean,mod_VPM_mean,mod_RF_mean),
               names_to = "Source", values_to = "gpp")
df_gpp_end2<-df_gpp_fit_final %>%
  select(doy,obs_mean,mod_conLUE_mean,mod_Pmodel_mean,
         mod_BESS_mean,mod_VPM_mean,mod_RF_mean)%>%
  pivot_longer(c(obs_mean,mod_conLUE_mean,mod_Pmodel_mean,
                 mod_BESS_mean,mod_VPM_mean,mod_RF_mean),
               names_to = "Source", values_to = "gpp_sm")
df_final<-left_join(df_gpp_end1,df_gpp_end2,by=c("doy","Source"))
df_final %>%
  ggplot()+
