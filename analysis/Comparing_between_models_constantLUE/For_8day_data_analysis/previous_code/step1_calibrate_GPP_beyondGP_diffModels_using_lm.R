...
# the calibration process is done for each site -->using liner regression at the end
############################################################
library(dplyr)
library(tidyr)
...
#also convert the gpp_obs to gpp_obs_8day to match other modelling gpp:
...
calibrate_fun<-function(df,model_name){
  ...
  gpp_name<-paste0("gpp_",model_name,"_8day")
  df_sel<-df %>%
 ...
 ##tidy the names of data.frame
  gpp_names<-c(paste0("cali_gpp_",model_name,"_8day"))
  df.cali<-df.cali %>%
...
gpp_vpm<-calibrate_fun(df_merge,"vpm")
#merge the datasets:
gpp_8day_final<-left_join(df_merge,
                       gpp_bess,
                       by=c("sitename","date"))

gpp_8day_final<-left_join(gpp_8day_final,
                           gpp_rf,
                           by=c("sitename","date"))
gpp_8day_final<-left_join(gpp_8day_final,
                           gpp_vpm,
...
save(gpp_8day_final,file = paste0(save.path,"GPP_from_diffSource_8day_cali.RDA"))
...
library(ggplot2)
gpp_8day_final_over<-gpp_8day_final[gpp_8day_final$Over_days_length>20 & !is.na(gpp_8day_final$Over_days_length),]
#
df_allsites<-gpp_8day_final_over %>%
  select(sitename,date,doy,sos,peak,gpp_obs_8day,starts_with("cali_"))%>%
  group_by(doy)%>%
  summarise(obs = mean(gpp_obs_8day, na.rm = TRUE),
            mod_cali_pmodel = mean(cali_gpp_pmodel_8day,na.rm=T),
            mod_cali_bess = mean(cali_gpp_bess_8day,na.rm=T),
            mod_cali_rf = mean(cali_gpp_rf_8day,na.rm=T),
            mod_cali_vpm = mean(cali_gpp_vpm_8day,na.rm=T),
