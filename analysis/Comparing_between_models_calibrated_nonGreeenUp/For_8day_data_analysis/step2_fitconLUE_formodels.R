...
load(paste0(load.path,"GPP_from_diffSource_8day_cali.RDA"))
...
fit_conLUE_fun_model<-function(df,model_name){
  ...
  gpp_name<-paste0("cali_gpp_",model_name,"_8day")
  LUE_allsites<-c()
  df_sel<-df%>%
  ...
  sites<-unique(df_sel_out_greenup$sitename)
  for (i in 1:length(sites)) {
    ...
    if(length(df_temp$gpp_cali[!is.na(df_temp$gpp_cali)])>5){
      ...
      names(LUE_temp)<-c("sitename","LUE","p-value") #the names seems not consistent,hence adding this line of code
    }
    if(length(df_temp$gpp_cali[!is.na(df_temp$gpp_cali)])<=5){
  ...
  gpp_names<-c(paste0("conLUE_gpp_",model_name,"_8day"))
  df.conLUE<-df_final %>%
    group_by(sitename)%>%
    mutate(gpp_mod_conLUE=APAR*conLUE_outGP) %>%   #calculated conLUE for all the periods
    mutate(gpp_mod_conLUE_8day=rollapply(gpp_mod_conLUE,width=8,FUN=function(x){mean(x,na.rm=T)},by=8,align="left",fill="NA"))%>%
    select(sitename,date,gpp_mod_conLUE_8day)
  #
...
fit_conLUE_fun_gppObs<-function(df){
  ...
  # df<-gpp_8day_final
  # ----fit the constant LUE for site(using data outside green-up period)--------
  gpp_name<-paste0("gpp_","obs","_8day")
  ...
  perc_values<-quantile(df_sel_out_greenup$gpp_obs_8day,probs = seq(0.1,1,0.1),na.rm = T)
  df_sel_out_greenup<-df_sel_out_greenup[df_sel_out_greenup$gpp_obs_8day>as.numeric(perc_values[5]),]
  for (i in 1:length(sites)) {
    #
    df_temp<-df_sel_out_greenup %>%
      filter(sitename==sites[i])
    #linear regression:
    if(length(df_temp$gpp_obs_8day[!is.na(df_temp$gpp_obs_8day)])>5){
      lm_norm<-lm(gpp_obs_8day~0+APAR,data=df_temp) #set the intercept =0
      stats_sum<-summary(lm_norm)
      LUE_temp<-data.frame("sitename"=sites[i],"LUE"=stats_sum$coefficients[1],"p-value"=stats_sum$coefficients[4])
      names(LUE_temp)<-c("sitename","LUE","p-value") #the names seems not consistent,hence adding this line of code
    }
  ...
  df_final<-left_join(df_sel,LUE_allsites,by="sitename")
  #tidy the names of the data.frame
  gpp_names<-c(paste0("conLUE_gpp_","obs","_8day"))
  df.conLUE<-df_final %>%
    group_by(sitename)%>%
    mutate(gpp_mod_conLUE=APAR*conLUE_outGP) %>%   #calculated conLUE for all the periods
    mutate(gpp_mod_conLUE_8day=rollapply(gpp_mod_conLUE,width=8,FUN=function(x){mean(x,na.rm=T)},by=8,align="left",fill="NA"))%>%
...
#---------------------------------------
#for bess,rf(same for the Fluxcom),pmodel, and vpm 
gpp_bess<-fit_conLUE_fun_model(gpp_8day_final,"bess")
gpp_rf<-fit_conLUE_fun_model(gpp_8day_final,"rf")
gpp_pmodel<-fit_conLUE_fun_model(gpp_8day_final,"pmodel")
gpp_vpm<-fit_conLUE_fun_model(gpp_8day_final,"vpm")
#and also for gpp_obs
gpp_obs<-fit_conLUE_fun_gppObs(gpp_8day_final)
#------------------
#merge the datasets with conLUE:
gpp_8day_conLUE<-left_join(gpp_pmodel,
                           gpp_bess,
                           by=c("sitename","date"))
gpp_8day_conLUE<-left_join(gpp_8day_conLUE,
                           gpp_rf,
                           by=c("sitename","date"))
gpp_8day_conLUE<-left_join(gpp_8day_conLUE,
                           gpp_vpm,
...
gpp_8day_conLUE_final<-left_join(gpp_8day_final,gpp_8day_conLUE,by=c("sitename","date"))
#save the data
save.path<-"D:/data/photocold_project/Merge_Data/Comparing_between_models_calibrated_nonGreeenUp/"
save(gpp_8day_conLUE_final,file = paste0(save.path,"GPP_from_diffSource_8day_conLUE.RDA"))
