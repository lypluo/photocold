...
Here need to note that the modelled GPP are updated p-model modelled GPP
...
#path to datasets
path<-"D:/data/photocold_project/Merge_Data/Using_sites_in_Fluxnet2015_updatePmodel/"
...
sep_data_indiffY_sepMismatch<-function(ddf,degf,Clim,PFT,site_name,avai_years){
  ...
  Years_unique<-unique(df$Year)
    ...
    #-------------------------------------------------------------------------------
    #update on Dec, 14th: update the names variables since the updated datasets sent by Beni changed the names
    #gpp_obs-->gpp; gpp_model_FULL-->gpp_mod
    gpp_max_obs<-quantile(df$gpp,0.95,na.rm = T)   
    gpp_max_mod<-quantile(df$gpp_mod,0.95,na.rm = T)
    df$gpp_mod_norm<-df$gpp_mod/gpp_max_mod
    df$gpp_obs_norm<-df$gpp/gpp_max_obs
    ...
    if(length(df$gpp[!is.na(df$gpp)])>30){
      df_new<-c()
    ...
    #-------------------------------------------------------------------------------
    #updated on Dec, 14, 2021-->same as before:
    #update some variable names:temp_day_fluxnet2015-->temp
    library(zoo)
    pos_diffYears<-c()
    ...
    df_subset<-df_subset%>%
        mutate(gpp_mod_roll5=rollapply(df_subset$gpp_mod_norm_f,5,mean,fill=NA,align="center"),
               ...
               temp_obs_roll20=rollapply(df_subset$temp,20,mean,fill=c(NA,"extend",NA),align="center")
               )
    ...
    df_stats<-subset(df_subset,is_event=="yes"&!is.na(gpp))
    df_stats$flag<-rep("overEsti",length(df_stats$date))
    for(k in 1:length(df_stats$date)){
      ifelse(df_stats$gpp_mod_FULL[k]-df_stats$gpp[k]<0,df_stats$flag[k]<-"underEsti",df_stats$flag[k]<-df_stats$flag[k])
    }
...
- Cfa-DBF (3 sites)
```{r echo = FALSE, message = FALSE, warning = FALSE,fig.height=10,fig.width=10}
#IT-Col-->select available complete-spring years and proper summer peak year:2005,2007:2011,2013:2014
site_name<-"IT-Col"
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Cfa_DBF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfa","DBF",site_name,c(2005,2007:2011,2013:2014))
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Cfa_DBF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfa","DBF",site_name,c(2003))
...
df_andPlot_Cfa_DBF3<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfa","DBF","US-MMS",c(2003:2014))
...
```{r echo = FALSE, message = FALSE, warning = FALSE,fig.height=10,fig.width=10}
#CN-Qia-->successful
df_andPlot_Cfa_ENF<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfa","ENF","CN-Qia",c(2004))
...
#DE-Hai-->successful
site_name<-"DE-Hai"
df_andPlot_Cfb_DBF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","DBF",site_name,c(2003:2011))
...
#DK-Sor-->successful
site_name<-"DK-Sor"
df_andPlot_Cfb_DBF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","DBF",site_name,c(2003,2005:2013))
...
p_merge<-plot_grid(p_diffYears[[1]],p_diffYears[[2]],p_diffYears[[3]],
          ...
          p_diffYears[[7]],p_diffYears[[8]],p_diffYears[[9]],
          p_diffYears[[10]],
          # p_diffYears[[11]],p_diffYears[[12]],p_diffYears[[13]],       
          labels = "auto",ncol=3,label_size = 12,align = "hv")
plot(p_merge)
...
#BE-Vie
site_name<-"BE-Vie"
df_andPlot_Cfb_MF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","MF",site_name,c(2003:2013))
...
#CH-Lae
site_name<-"CH-Lae"
df_andPlot_Cfb_MF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","MF",site_name,c(2005:2013))
...
#DE-Lkb
site_name<-"DE-Lkb"
df_andPlot_Cfb_ENF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2010:2012))
...
#DE-Obe
site_name<-"DE-Obe"
df_andPlot_Cfb_ENF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2009:2013))
length(df_andPlot_Cfb_ENF2$p_isevent)
p_diffYears<-df_andPlot_Cfb_ENF2$p_isevent
p_merge<-plot_grid(p_diffYears[[1]],p_diffYears[[2]],p_diffYears[[3]],
          p_diffYears[[4]],p_diffYears[[5]],
          # p_diffYears[[6]],p_diffYears[[7]],
          labels = "auto",ncol=3,label_size = 12,align = "hv")
plot(p_merge)

#DE-Tha
site_name<-"DE-Tha"
df_andPlot_Cfb_ENF3<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2003:2013))
...
#FR-LBr
site_name<-"FR-LBr"
df_andPlot_Cfb_ENF4<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2004:2008))
...
#IT-Lav-->successful
site_name<-"IT-Lav"
df_andPlot_Cfb_ENF5<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2004:2014))
...
#NL-Loo
site_name<-"NL-Loo"
df_andPlot_Cfb_ENF6<-sep_data_indiffY_sepMismatch(ddf,0.05,"Cfb","ENF",site_name,c(2003:2012))
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Dfb_DBF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","DBF",site_name,c(2004))
...
#US-UMB
site_name<-"US-UMB"
df_andPlot_Dfb_DBF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","DBF",site_name,c(2003:2013))
...
#US-UMd
site_name<-"US-UMd"
df_andPlot_Dfb_DBF3<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","DBF",site_name,c(2008:2013))
...
#US-Ha1-->successful
site_name<-"US-Ha1"
df_andPlot_Dfb_DBF4<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","DBF",site_name,c(2003:2012))
...
#US-Wcr
site_name<-"US-WCr"
df_andPlot_Dfb_DBF5<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","DBF",site_name,c(2003:2006,2011:2013))
...
p_merge<-plot_grid(p_diffYears[[1]],p_diffYears[[2]],p_diffYears[[3]],
          ...
          p_diffYears[[7]],
          # p_diffYears[[8]],
          # p_diffYears[[9]],p_diffYears[[10]],p_diffYears[[11]],
          labels = "auto",ncol=3,label_size = 12,align = "hv")
plot(p_merge)
...
#US-PFa-->successful
site_name<-"US-PFa"
df_andPlot_Dfb_MF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","MF",site_name,c(2003:2004,2006:2013))
...
#US-Syv
site_name<-"US-Syv"
df_andPlot_Dfb_MF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","MF",site_name,c(2003:2006,2013:2014))
...
#CZ-BK1-->successful
site_name<-"CZ-BK1"
df_andPlot_Dfb_ENF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","ENF",site_name,c(2005:2007))
...
#RU-Fyo-->successful
site_name<-"RU-Fyo"
df_andPlot_Dfb_ENF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfb","ENF",site_name,c(2003:2013))
...
p_merge<-plot_grid(p_diffYears[[1]],p_diffYears[[2]],p_diffYears[[3]],
          ...
          p_diffYears[[7]],p_diffYears[[8]],p_diffYears[[9]],
          p_diffYears[[10]],p_diffYears[[11]],
          # p_diffYears[[12]],p_diffYears[[13]],p_diffYears[[14]],        
          labels = "auto",ncol=3,label_size = 12,align = "hv")
print(p_merge)
...
#CA-Man
site_name<-"CA-Man"
df_andPlot_Dfc_ENF1<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003,2007))
...
#CA-NS1-->successful
site_name<-"CA-NS1"
df_andPlot_Dfc_ENF2<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2004))
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Dfc_ENF3<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2004))
...
#CA-NS3-->succesful
site_name<-"CA-NS3"
df_andPlot_Dfc_ENF4<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2004))
length(df_andPlot_Dfc_ENF4$p_isevent)
p_diffYears<-df_andPlot_Dfc_ENF4$p_isevent
p_merge<-plot_grid(p_diffYears[[1]],p_diffYears[[2]],
          # p_diffYears[[3]],p_diffYears[[4]],
          labels = "auto",ncol=3,label_size = 12,align = "hv")
print(p_merge)
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Dfc_ENF5<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003))
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Dfc_ENF6<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2004))
...
#CA-Qfo
site_name<-"CA-Qfo"
df_andPlot_Dfc_ENF7<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2004:2009))
...
#FI-Hyy
site_name<-"FI-Hyy"
df_andPlot_Dfc_ENF8<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2013))
...
##IT-Ren
site_name<-"IT-Ren"
df_andPlot_Dfc_ENF9<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003,2005:2012))
...
df<-ddf %>%
  dplyr::filter(sitename==site_name)
df_andPlot_Dfc_ENF10<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003))
...
#US-NR1-->successful
site_name<-"US-NR1"
df_andPlot_Dfc_ENF11<-sep_data_indiffY_sepMismatch(ddf,0.05,"Dfc","ENF",site_name,c(2003:2014))
