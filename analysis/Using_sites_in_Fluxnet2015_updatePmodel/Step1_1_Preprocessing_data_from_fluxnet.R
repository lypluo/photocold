##########################################
#Try to preprocess the fluxnet data
##########################################
#------------------------------------
#(1)for fluxnet data:
#------------------------------------
...
#
load(paste0(save.path,"Daily_data.RDA"))
#----------------------------------------------------
...
#updated in 2021-12-14: in this folder-->using the update p-model gpp-->using the model_data.rds from 
#"gpp_bias_correction" project
#----------------------------------------------------
...
df_Beni_sel$date<-as.Date(df_Beni_sel$date)
#----------------
...
compare_vars<-function(df_YP_daily,df_Beni_daily,var_in_YP,var_in_Beni){
  ...
  df_merge<-left_join(df_YP_daily_sel,df_Beni_daily_sel,by = c("sitename","date"))
...
#for GPP-->working here
p_gpp<-compare_vars(df_all_sel_daily,df_Beni_sel,"GPP_NT_mean","gpp")
p_ppfd<-compare_vars(df_all_sel_daily,df_Beni_sel,"PPFD_IN_mean","ppfd")
p_temp<-compare_vars(df_all_sel_daily,df_Beni_sel,"Ta_mean","temp")
p_preci<-compare_vars(df_all_sel_daily,df_Beni_sel,"P","prec")
p_patm<-compare_vars(df_all_sel_daily,df_Beni_sel,"PA_mean","patm")
p_vpd<-compare_vars(df_all_sel_daily,df_Beni_sel,"VPD_day_mean","vpd")
