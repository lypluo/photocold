...
load.path<-"D:/data/photocold_project/FLUXNET_data/Using_sites_in_Fluxnet2015/"
...
#load the new p-model gpp data
load.path<-"D:/Github/photocold/data/"
df_Beni<-readRDS(file=paste0(load.path,"model_data.rds"))
#since Beni's new datasets do not have the fapar, then load the original old datasets to merge the fapar:
df_Beni_Old<-read.csv(file=paste0(load.path,"ddf_fluxnet2015_pmodel_with_forcings_stocker19gmd.csv"))
pos_fapar<-grep("fapar_",names(df_Beni_Old))
df_sel_fapar<-df_Beni_Old[,c(1:2,pos_fapar)]
df_sel_fapar$date<-as.Date(strptime(df_sel_fapar$date,format = "%m/%d/%Y"))
#merge the data:
df_Beni<-left_join(df_Beni,df_sel_fapar,by=c("sitename","date"))
...
df_avai<-df_Beni %>%
  filter(sitename %in% avai.sites)
df_avai$date<-as.Date(df_avai$date)
...
ifelse(length(pos_del)>0,df_YP_daily_temp<-df_YP_daily[,-pos_del],df_YP_daily_temp<-df_YP_daily)
...
save.path<-"D:/data/photocold_project/Merge_Data/Using_sites_in_Fluxnet2015_updatePmodel/"
